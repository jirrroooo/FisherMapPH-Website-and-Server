...
export default function ManageDistressCalls() {
  ...
  function getData() {
    fetch(`${useApiStore.getState().apiUrl}reports?status=archive`, {
  ...
  function getFilteredData() {
    ...
    if (sortBy != "Sort By" && searchBy != "Search by") {
      fetch(
        ...
        }reports?sort=${sortBy}&searchBy=${searchBy}&search=${search}&page=${page}&status=archive`,
        {
          headers: {
      ...
      fetch(
        `${
          useApiStore.getState().apiUrl
        }reports?searchBy=${searchBy}&search=${search}&page=${page}&status=archive`,
        {
          headers: {
            Authorization: `Bearer ${useLoginStore.getState().token}`,
          },
  ...
  function getDataByPage(pageNumber) {
    ...
    fetch(`${useApiStore.getState().apiUrl}reports?page=${pageNumber}&status=archive`, {
      headers: { Authorization: `Bearer ${useLoginStore.getState().token}` },
    })
      .then((response) => response.json())
      .then((body) => {
  ...
  function getFilteredDataByPageNumber(pageNumber) {
    ...
    if (sortBy != "Sort By" && searchBy != "Search by") {
      fetch(
        `${
          useApiStore.getState().apiUrl
        }reports?sort=${sortBy}&searchBy=${searchBy}&search=${search}&page=${pageNumber}&status=archive`,
        {
          headers: {
            Authorization: `Bearer ${useLoginStore.getState().token}`,
          },
      ...
      fetch(
        `${
          useApiStore.getState().apiUrl
        }reports?searchBy=${searchBy}&search=${search}&page=${pageNumber}&status=archive`,
        {
          headers: {
            Authorization: `Bearer ${useLoginStore.getState().token}`,
          },
      ...
      {!isLoading ? (
        <>
          ...
          <div className="container mt-4 text-center">
            <h2>Archive | Distress Calls Logs</h2>
            <p>
