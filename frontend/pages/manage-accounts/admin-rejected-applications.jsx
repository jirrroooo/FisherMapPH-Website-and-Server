...
export default function AdminRejectedApplications() {
  ...
  function getDataByPage(pageNumber) {
    ...
    fetch(
      `${
        useApiStore.getState().apiUrl
      }users/admin-rejected-users?page=${pageNumber}`,
      {
        headers: { Authorization: `Bearer ${useLoginStore.getState().token}` },
      }
    )
  ...
  function getFilteredDataByPageNumber(pageNumber) {
    ...
    if (sortBy != "Sort By" && searchBy != "Search by") {
      fetch(
        `${
          useApiStore.getState().apiUrl
        }users/admin-rejected-users?sort=${sortBy}&searchBy=${searchBy}&search=${search}&page=${pageNumber}`,
        {
          headers: {
            Authorization: `Bearer ${useLoginStore.getState().token}`,
          },
      ...
      fetch(
        `${
          useApiStore.getState().apiUrl
        }users/admin-rejected-users?searchBy=${searchBy}&search=${search}&page=${pageNumber}`,
        {
          headers: {
            Authorization: `Bearer ${useLoginStore.getState().token}`,
          },
